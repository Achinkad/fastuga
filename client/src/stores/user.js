...
export const useUserStore = defineStore("user", () => {
  ...
  async function login(credentials) {
    try {
      ...
      socket.on("loggedIn", () => {
        toast.success(
          "User " + user.value.name + " has entered the application."
        );
      });
      if (user.value.type == "C") {
        await get_customer(user.value);
      }
      return true;
    } catch (error) {
      clearUser();
  ...
  function save(user_values, user_id) {
    axios
      .put(serverBaseUrl + "/api/users/" + user_id, user_values)
      .then((response) => {
        //user.value = response.data.data
      ...
      .catch((error) => {
        ...
        if (error.response.status == 422) {
          toast.error(
            "User #" + user_id + " was not updated due to validation errors!"
          );
          errors.value = error.response.data.data;
        } else {
          toast.error(
            "User #" + user_id + " was not updated due to unknown server error!"
          );
        }
      });
  }
  ...
  const get_customers_this_month = () => {
    return number_customers_this_month.value;
  };
